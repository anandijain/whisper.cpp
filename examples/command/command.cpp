    ...
    while (is_running)
    {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy))
            {
                ...
                // we are directly listening for voice commands
                audio.get(params.command_ms, pcmf32_cur);

                const auto txt = ::trim(::transcribe(ctx, params, pcmf32_cur, prob, t_ms));

                std::string command = txt;
                auto to_server = command + "\n";
                
                // Create and connect the socket inside the loop
                int client_socket;
                ...
                int connection_status = connect(client_socket, (sockaddr *)&server_address, sizeof(server_address));
                ...
                send(client_socket, to_server.c_str(), to_server.size(), 0);

                fprintf(stdout, "stinky boi");
                fprintf(stdout, "%s: Command '%s%s%s', (t = %d ms)\n", __func__, "\033[1m", command.c_str(), "\033[0m", (int)t_ms);
                fprintf(stdout, "\n");
